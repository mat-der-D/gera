...
> **いまはまだ、書いた CSS を読み込ませる仕組みが無い。**ユーザー CSS の読み込みは実装順序 7（設計文書 第14節）で、この文書はその一つ前——**名前を決めて公開する**段階のものである。いま試すなら開発時の `npm run dev` で `src/viewer.css`（閲覧モードの版面）・`src/style.css`（色と書体）・`src/outline.css`（見出しの一覧）を直接いじることになる。
