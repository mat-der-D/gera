 ...
 * 3. **ファイル名の隣の案内**（`.gera-keys-tip`。style.css と viewer.ts）——本文の
 *    先頭にあるので、送れば一緒に流れて消える
 *
 ...
 * （`scrollDOM`）で、`.gera-file` と同じ形の絶対配置である——**本文に差し込むと
 * 行の高さの計算がずれてカーソルの位置が狂う**（style.css）。`pointer-events` も
 * 切ってあるので、文字の選択も奪わない。
 *
