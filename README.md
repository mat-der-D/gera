...
| モード | 位置づけ | 表示 | 対応する作業 |
|---|---|---|---|
| 閲覧 | **本体** | 完全に描画する。表・数式・コードブロック（色付き） | 読み合わせ、調べ物 |
| 編集 | 従 | 記法を残したまま装飾のみ当てる | 削除と並べ替え |

`Mod+E` で切り替える。**切り替え前に見ていた箇所が、切り替え後も画面に見えている**——これを両方向で満たす。
...
- **highlight.js** — コードブロックの色。**色は CSS 側にあり**、`user.css` で変えられる（[docs/CSS.md](docs/CSS.md)）
...
**初版のあと、シンタックスハイライトを足した**（2026-09-08。[docs/DESIGN.md](docs/DESIGN.md) 9-15）。**載せる言語は本人の文書 78,231 本を数えて選んだ**——JSON・シェル・YAML・TOML・Markdown・JavaScript・TypeScript・Rust・Python・CSS・HTML・SQL・差分。**知らない言語には色を付けない。**

初版に入れない機能（ファイル横断検索など）は**後から足すつもりのものであって、却下したものではない。**
