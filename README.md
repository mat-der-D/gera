...
| モード | 位置づけ | 表示 | 対応する作業 |
|---|---|---|---|
| 閲覧 | **本体** | 完全に描画する。表・数式・コードブロック（色付き）・Mermaid の図 | 読み合わせ、調べ物 |
...
- **Mermaid** — 図。**図が画面に近づいたときだけ読み込む。**色は本文の色から作るので、`dark.css` を置けば図も暗くなる
...
**初版のあと、シンタックスハイライトと Mermaid の図を足した**（2026-09-08。[docs/DESIGN.md](docs/DESIGN.md) 9-15）。**載せる言語は本人の文書 78,231 本を数えて選んだ**——JSON・シェル・YAML・TOML・Markdown・JavaScript・TypeScript・Rust・Python・CSS・HTML・SQL・差分。**知らない言語には色を付けない。**

**Mermaid は、図が画面に近づいたときに初めて読み込む。**図の無い文書は一切触らない——mermaid は gera 全体より大きいので、起動経路にも通常の閲覧にも乗せていない。**バイナリは 5.92 MB から 6.81 MB になった**（+0.87 MB）。
